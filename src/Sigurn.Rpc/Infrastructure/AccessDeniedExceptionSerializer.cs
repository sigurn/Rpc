...
class AccessDeniedExceptionSerializer : ITypeSerializer<AccessDeniedException>
{
    ...
    {
        var message = await Serializer.FromStreamAsync<string>(stream, context, cancellationToken).ConfigureAwait(false);
    ...
    {
        await Serializer.ToStreamAsync(stream, value.Message, context, cancellationToken).ConfigureAwait(false);
    }
}